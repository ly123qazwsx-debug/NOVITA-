...
1. **成本概览数据**：4 个 KPI（当月总消耗、日消耗含固定、日消耗按需、预计全月）+ 累计/预计对比、日均对比
2. **分项环比明细**：用 8 月表底「当期合计 / 上月同期 / 环比率」画对比柱 + 明细表（上月同期不含 sd 7.12-7.14 异常）
3. **五项成本日度趋势**：GPU 固定用柱（左轴），LLM / sd / GPU按需 / 存储用折线（右轴）；**每天都标出金额**，下方还有按日明细表（五项 + 当日合计）

标题会写明「今天 X 月 X 日，统计区间 1 日–昨天」。红涨绿跌。若表里有「预计X月总消耗」数字，预计全月优先用表上的值，而不是简单按日线性外推。
