...
每天 **11:10**（北京时间）自动从飞书 Wiki 拉取 **NOVITA** 与 **AWS** 工作表，生成看板并推送到飞书群。
...
报告末尾会自动附上 **每周消耗汇总** 与 **周度异常**（按每月 1–7、8–14… 自然周划分，对比上周与上月同周）。

## 报告口径（截至昨日）

| 指标 | 计算方式 |
|------|----------|
| 统计区间 | **本月 1 日 ～ 昨天**。例如今天 8 月 21 日，只统计 8 月 1 日～8 月 20 日，不含当天 |
| 成本概览（第一板块） | **直接用飞书表里的对比表**：当月 / 7 月 / 环比 / 环比率。7 月总消耗、预计对比都取这张表，不按 7 月逐日加总 |
| 分项环比明细 | **优先用当月表底**「当期合计 / 上月同期合计 / 环比率」（已剔除 sd 7.12-7.14 异常） |

分项环比明细：LLM、sd、GPU (按需)、GPU (按需存储)、GPU 固定。

## 一张综合看板（深色，两段组合）

飞书每天只推 **1 张图**，深色底、主色为亮黄 `#FFD400`（对齐 Digen Playground），标题和图表放大，图上金额**不带 $ 符号**，**每一块都标明单位：美元**：

1. **4 个 KPI 卡片**：当月总消耗、日消耗（含固定 GPU）、日消耗（按需）、预计全月
2. **分项环比明细表**：用当月表底「当期合计 / 上月同期 / 环比率」（上月同期不含 sd 7.12-7.14 异常）
3. **日度趋势（拆成两张图，避免量级差太大）**
   - 主要成本：GPU 固定用柱（左轴），LLM / sd 用折线（右轴），**每个点都标当天金额**
   - 按需成本：GPU 按需、GPU 按需存储单独放大，**每个点都标当天金额**

标题为「NOVITA X月成本概览（截至X/X）」，底部附口径说明与数据来源。

## AWS 看板与汇总

同一 Wiki 文档内的 **AWS** 工作表会生成第二张看板（粉色主题）：

1. **3 个 KPI**：当月总消耗、日消耗、预计全月
2. **累计消耗前 10 项** 分项环比表
3. **日度趋势**：排名 1~5 / 6~10 两组折线图（与上传模板一致，不含 AWS 总费用线）
...
1、当期总消耗（8.1–8.25）：$14,116.08，环比上涨 5%。
2、日消耗：$564.64，环比上涨 5%。
3、预计 8 月总消耗：$17,148.69，环比上涨 4%。

其中：
【ELB 负载均衡】：从 8 月 15 日起，消耗连续上涨。重点检查...
```

`config.yaml` 的 `aws.insights.watch_services` 可配置重点服务与检查提示；脚本会自动检测从指定日期起是否连续上涨。
...
底部「当期合计 / 上月同期 / 环比 / 环比率」行会自动跳过。读取范围默认 `A1:Z400`，以覆盖全年，并读到表右侧的对比表 / 预计 / 实际汇总。
